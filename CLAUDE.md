...
> **Nombres:** empresa = **Pulso Digital**. Producto = **Radar de Tracción** (tentativo). Repo = `pulso-digital` (renombrado desde `tiktok_dashboard` el 2026-07-08; la URL de Pages cambió).
>
...
- **Implicancia técnica #1 — el acceso NO está decidido ❓:** hoy todo es público. Si mañana hay un cliente pagando, lo que él ve no puede quedar público para su adversario. Pero *cuál* es la arquitectura (vitrina + paneles privados / teaser + muro / sin muro, vendiendo solo servicio) **está abierto** y depende del modelo de negocio, que también está abierto. Ver `contexto-maestro.md §5`. No asumir la opción de paneles privados. Fase 8 existe para cuando se decida.
...
- **Nota de migración:** el proyecto nació 100% en GitHub (Pages público). Si el modelo elegido exige acceso privado, habrá que migrar a un hosting que lo soporte. Es trabajo conocido; está como Fase 8, **EN PAUSA hasta que haya un cliente pagando**. Por ahora **no** se toca el desarrollo salvo que la persona lo pida.
...
**Fase 8 — Acceso privado / monetización (migración). ⏸️ EN PAUSA — no ejecutar hasta que haya un cliente pagando y el modelo de negocio esté elegido.** Preparar el terreno para vender: separar **vitrina pública** (panel básico gratis, gancho) de **paneles privados por cliente** (filtrados por su coalición/figura, con informe). Requiere salir de "todo público en GitHub Pages" hacia un hosting con autenticación. *Decisiones a tomar cuando lleguemos:* dónde alojar (mantener Pages para la vitrina + app privada aparte, o migrar todo), cómo autenticar (simple primero), y cómo aislar los datos de cada cliente. Empezar simple; no sobre-ingenierizar antes de tener el primer cliente pagando.
...
- [x] Fase 4 — desplegado en GitHub (Actions + Pages) *(2026-07-07: workflow `snapshot.yml` corre diario a las 12:00 UTC (~08:00 Chile) + lanzamiento manual; `APIFY_TOKEN` en Secrets; repo pasado a público para poder usar Pages gratis; Pages sirviendo `main` → `/docs`; dashboard en vivo — URL actualizada tras el renombre a `pulso-digital`)*
...
*(2026-07-09, cliente Apify asíncrono — fix de timeout:*
- *`src/apify_client.py` pasó de llamada SÍNCRONA a **run asíncrono + polling**. Motivo: el endpoint síncrono topa a los ~300s y con 49 cuentas TikTok + 52 IG el run quedaba TIMED-OUT (el workflow venía fallando con 408 run-timeout-exceeded).*
- *Ahora dispara el run con **`timeoutSecs` explícito y configurable** (default 1800), consulta el estado cada `poll_cada_s` (default 10) hasta un estado terminal, y solo con SUCCEEDED lee el dataset. Timeouts por CLI (`--timeout-run`, `--poll`), sin números mágicos.*
- ***TikTok e Instagram corren como runs INDEPENDIENTES**; cada snapshot se guarda apenas termina su actor. Si IG falla, TikTok no se cae (el código de salida refleja solo TikTok).*
- ***Esquema del actor de IG: VERIFICADO** con handles reales el 2026-07-08 (crudo `data/snapshots/raw/2026-07-08T1813-ig-raw.json`, 52 perfiles): `username`→handle, `followersCount`→followers, `followsCount`→following, `postsCount`→posts, `verified`, `private`. Regla §6 cumplida, no asumido.*
- ***28 tests en verde** (3 nuevos del cliente async con `requests`/`sleep` mockeados: éxito, TIMED-OUT y run que no termina).*
- ***⚠️ Primera corrida con el fix (workflow 2026-07-09): job en verde pero INCOMPLETA.** El snapshot TikTok trae **31 de 49 cuentas** (`data/snapshots/2026-07-09.json`; faltan 18, entre ellas pamelajilestv, jorgealessandridiputado, manueljoseossandon, macasantelices, ximenaossandon.oficial, ignacio.achurra…) y **NO se generó snapshot de Instagram** ese día (el run IG no produjo dataset). El job igual pasó porque una falla de IG no lo tumba y TikTok devolvió SUCCEEDED con datos parciales. **Pendiente de diagnóstico:** ¿el actor de TikTok devolvió menos ítems (rate-limit/cuentas caídas) o el run cerró antes de traerlas todas? ¿por qué IG no dejó snapshot? Revisar en la Console la corrida del 07-09 (nº de resultados de cada run) antes de darla por buena.)*
...
*(2026-07-08, sesión del hilo director: (a) **repo renombrado** `tiktok_dashboard` → `pulso-digital`; actualizar cualquier enlace guardado. (b) **Tarjeta habilitada en Apify** — la corrida ya no se corta al superar el free tier de $5. (c) El foco del proyecto pasó de técnico a comercial: **el desarrollo está en pausa**. Las fases 5, 7, 8, 9 y 10 no se ejecutan hasta que la validación de mercado diga hacia dónde. (d) **Nada del modelo de negocio está decidido** — ni qué se vende, ni si el dashboard público es vitrina o regalo. Si una tarea técnica asume una respuesta a eso, está mal planteada: preguntar antes.)*
