...
*(2026-07-09, fix de timeout: el workflow falló con 408 run-timeout-exceeded / run TIMED-OUT — el endpoint SÍNCRONO de Apify no aguanta 49 TikTok + 52 IG. `apify_client.py` reescrito a modo ASÍNCRONO + polling: dispara el run con `timeoutSecs` explícito (default 1800), consulta estado cada 10s hasta terminal, y recién con SUCCEEDED lee el dataset. Timeouts configurables por CLI (`--timeout-run`, `--poll`), sin números mágicos. TikTok e IG son runs independientes que guardan su snapshot apenas terminan; si IG falla no tumba TikTok. El esquema IG quedó verificado con el crudo real (`followersCount`/`followsCount`/`postsCount`/`verified`/`private`). El run TIMED-OUT del workflow se descarta: ya existe el snapshot bueno del 2026-07-08. 28 tests en verde (3 nuevos del cliente asíncrono con requests mockeado). Pendiente: Álvaro relanza el workflow tras confirmar costo.)*
